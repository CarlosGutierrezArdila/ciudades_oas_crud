...
import (
	"encoding/json"
	"errors"
	"oas_ciudad_pais/ciudades_crud/models"
	"strconv"
	"strings"
...
// URLMapping ...
func (c *PaisController) URLMapping() {
...
func (c *PaisController) Post() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddPais(&v); err == nil {
...
func (c *PaisController) GetAll() {
	...
	l, err := models.GetAllPais(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
		//c.Data["development"] = map[string]interface{}{"Code": "000", "Body": err.Error(), "Type": "error"}
		c.Data["system"] = err
...
func (c *PaisController) Put() {
	...
	v := models.Pais{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdatePaisById(&v); err == nil {
			c.Data["json"] = v
		} else {
			logs.Error(err)
			//c.Data["development"] = map[string]interface{}{"Code": "000", "Body": err.Error(), "Type": "error"}
...
func (c *PaisController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeletePais(id); err == nil {
